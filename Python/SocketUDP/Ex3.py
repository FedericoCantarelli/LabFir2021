# First Example: see readme.txt for details
from socket import *
...
try:
      ...
      modMessage, serverAddress = clientSocket.recvfrom(2048)     # 2048 is the max dim in bytes
                                                            # both modMessage and server address are returned from the method

      # Note that recvfrom() does not explicit IP address                              
      modMessage = modMessage.decode('utf-8')
      print("The modified string is: " + modMessage, "\n Get from ", serverAddress)
